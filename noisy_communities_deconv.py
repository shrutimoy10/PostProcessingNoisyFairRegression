...
protected_columns = ["ispctblacklarge"]
...
def generate_proxy_columns(df, noise_param=1):
    ...
    for j in noise_idx:
        ...
        if num_groups > 1:
          ...
          if df_proxy[protected_columns[0]][j] == 1:
                  df_proxy.at[j, protected_columns[0]] = 0
                  # group_index = i
                  # allowed_new_groups = list(range(num_groups))
                  # allowed_new_groups.remove(group_index)
                  # new_group_index = random.choice(allowed_new_groups)  
            ...
            df_proxy.at[j, protected_columns[0]] = 1
...
for noise_level in gamma :  
  ...
  df_protected_column_names = ['ispctblacklarge']
...
plt.legend()
plt.savefig("communities/KS_dist_test_deconv_comm.pdf", dpi=200)
plt.close()

plt.plot( diff_in_w, ".", linestyle="--",label="w estimate error")
plt.xticks(np.arange(len(gamma)), gamma)
plt.xlabel("$\\epsilon$")
plt.ylabel("$||w - w^{est}||_1$")
# plt.title("W Estimation Error")
# plt.legend()
plt.savefig("communities/w_est_error_test.pdf", dpi=200)
plt.close()